...
# inside legacy Office files). Such strings surface before real document
# text in byte order and once filled the output cap alone (a 5MB .ppt of
# photographed slides extracted as pure Photoshop metadata). Never user
# content — always safe to drop.
_OLE_METADATA_MARKERS = frozenset({
    ...
    # (never slide content, which lives in slide*.xml — absent here).
...
def _has_seq_run(text: str, run_len: int = _SEQ_RUN_LEN) -> bool:
...
def _is_ole_garbage(text: str, strict: bool = False) -> bool:
    ...
    try:
        ...
        if any(m in low for m in _OLE_METADATA_MARKERS):
            return True
        if not text:
            return True
        if _has_seq_run(text):
        ...
        if len(text) < 30:
            ...
            if _LOWER_WORD_RE.search(text) is None:
                return True
            return any(not (ch.isalnum() or ch in _OLE_PROSE_EXTRA)
                       for ch in text)
        odd = sum(1 for ch in text
                  if not (ch.isalnum() or ch in _OLE_PROSE_EXTRA))
