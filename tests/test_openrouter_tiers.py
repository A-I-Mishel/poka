...
(curated, user pick), then the local Ollama tail. Free Router + GLM 5.2
removed Sep 2026 (local tier takes the fallback role; GLM failed its
trial on rate limits); Qwen 3.8 27B removed (trial ended, per user
request); Ling 3.0 Flash VL removed (per user request).
"""
...
def test_no_key_means_skipped(monkeypatch):
    ...
    assert not hasattr(config, "get_tier_openrouter_ling_vl_llm")
    assert not hasattr(config, "OPENROUTER_QWEN_MODEL")
    assert not hasattr(config, "OPENROUTER_LING_VL_MODEL")
