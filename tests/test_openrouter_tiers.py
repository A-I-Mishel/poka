...
(curated, user pick) ahead of the Qwen/Ling trial lanes (Sep 2026),
Ling VL last. Free Router + GLM 5.2 removed Sep 2026 (local tier takes
the fallback role; GLM failed its trial on rate limits).
"""
...
def test_clients_built_with_key(monkeypatch):
    ...
    assert qwen is not None and ling is not None
...
def test_trial_lanes_full_synthesis_members():
    """Qwen/Ling trial lanes: full synthesis members, never cheap."""
    synth = [name for name, _ in config.SYNTHESIS_TIERS]
    cheap = [name for name, _ in config.CHEAP_TIERS]

    for lane in ("OpenRouter Qwen 27B", "OpenRouter Ling VL"):
