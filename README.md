...
| `OPENROUTER_API_KEY` | no | OpenRouter trial lanes (Nemotron Ultra, Ling VL) |
...
10. OpenRouter Ling 3.0 Flash VL (`inclusionai/ling-3.0-flash-vl:free` via `OPENROUTER_LING_VL_MODEL`;
    vision-capable trial Sep 2026 — free vision fallback behind Gemini/Cohere)
11. Ollama qwen3:8b (local offline fallback tail; `OLLAMA_MODEL`, disable via `OLLAMA_ENABLED=false`)

Fast mode answers use only lanes 6 + 9–11 (Gemini 3.1 Flash Lite,
Nemotron Ultra, Ling VL, local Ollama 8B) and fail honestly when all
four are down; Deep Mode uses the full cascade above. TokenHarbor MiMo
2.6 Flash + DeepSeek V4.1 Flash removed (trial ended); OpenRouter Qwen
3.8 27B removed (trial ended, per user request).
...
(Gemma/Super/3.5/26B/Ling-Fin/Laguna) — only Nemotron Ultra + Ling
trial lane (Sep 2026, remove if flaky; GLM already removed after failing
its trial, Qwen 27B removed per user request) kept.
